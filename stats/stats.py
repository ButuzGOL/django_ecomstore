...
def recommended_from_views(request):
    ...
    if viewed:
        productviews = ProductView.objects.filter(product__in=viewed). \
                                                        values('tracking_id')
        t_ids = [v['tracking_id'] for v in productviews]
        # if there are other tracking ids, get other products.
        if t_ids:
            all_viewed = Product.active. \
                                    filter(productview__tracking_id__in=t_ids)
            # if there are other products, get them, excluding the
            # products that the customer has already viewed.
            if all_viewed:
                other_viewed = ProductView.objects. \
                    filter(product__in=all_viewed).exclude(product__in=viewed)
                if other_viewed:
                    return Product.active. \
                                filter(productview__in=other_viewed).distinct()
